...
export default function Inspect({ today, clock }) {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const res = await axios.get(
                `http://${window.location.host}/api/inspect`,
                {
                    params: { search, date, time }
                }
            );

            console.log("Inspect API Response:", res.data); // 🔍 debug
    ...
    // Safely get current snapshot
    const currentSnapshot =
        history.length > 0 && history[currentIndex]
            ? history[currentIndex]
            : null;

    const currentPc =
        pcData && currentSnapshot
            ? {
                ...pcData,
    ...
    const currentTimestamp =
        currentSnapshot?.timestamp
            ? new Date(currentSnapshot.timestamp)
            : null;
    ...
    return (
        <>
            ...
            <div className="flex items-center gap-4 p-4 text-white w-full">
                ...
                <input
                    type="text"
                    placeholder="Search by PC ID or Hostname"
                    value={search}
            ...
            {history.length > 0 && currentSnapshot && (
                <div className="px-8 mt-6 text-white relative">
                    ...
                    <div className="flex justify-between text-xs text-gray-400 mt-2">
                        <span>
                            {new Date(history[0]?.timestamp || 0).toLocaleString()}
                        </span>
                        <span>
                            {new Date(
