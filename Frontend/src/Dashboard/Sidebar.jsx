...
const menuItems = [
    ...
    { icons: <NotepadText size={20} />, label: "Report", path: "/report" },
...
export default function Sidebar() {
    ...
    return (
        <aside className="h-screen shrink-0">
            <nav className={`shadow-md h-screen p-2 flex flex-col duration-500 bg-blue-800 text-white ${open ? 'w-60' : 'w-16'}`}>
                    ...
                    {menuItems.map((item, index) => {
                        return (
                            <NavLink to={item.path} key={index} className={({ isActive }) => `px-3 py-2 my-2 rounded-md duration-300 cursor-pointer flex gap-2 items-center relative group
                                ...
                                {({ isActive }) => (
                                    <>
                                        <div className={isActive ? "text-black" : "text-white"}>{item.icons}</div>
                                        <p className={`${!open && "w-0 translate-x-24"} duration-500 overflow-hidden ${isActive ? "text-black" : "text-white"}`}>{item.label}</p>
                ...
                <div className='flex items-center gap-2 p-2 hover:bg-blue-500 rounded-4xl duration-500 border border-white' onClick={() => setOpen(!open)}>
